...
def  main():
  ...
  installer = "\installer\Pandura Installer.exe"
   
  if demo:
    installer = "\installer\Pandura Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Pandura manual.pdf" 
  ]
  ...
  ZIPNAME = "Pandura-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Pandura-v" + FULLVERSIONSTR + "-win-demo.zip"
